...
raw_save_dir = save_dir + "/raw_measures"
...
def main():
    global reading_q, map_q, heatmap_generator, heatmap, current_heatmap_saves, current_direction_saves, quant_levels
    global heatmap_generator_arr_x, heatmap_generator_arr_y, interp_arr_x, interp_arr_y
    ...
    print_wt(client_name + " subscribed to topic: " + topic_receive_bin)

    cleanup = CleanupThread(path=source_dir,
                            save_path=raw_save_directory)
    cleanup.start()
    ...
    while True:
        ...
        if keyboard.is_pressed('c'):
            print_wt("C is pressed, initiating cleanup...")
            cleanup.cleanup()
        time.sleep(0.1)
